...
class _ScanReceiptScreenState extends State<ScanReceiptScreen> {
  ...
  /// before OCR Ч it turns barely-visible gray text into sharp black.
    ...
    try {
      ...
      if (CloudOcrService.isConfigured) {
        try {
          ...
          ocrLog = e.message;
          // Cloud failed Ч fall through to ML Kit
        } catch (e) {
          ocrSource = 'Yandex Vision (исключение)';
          ocrLog = e.toString();
        }
      } else {
        ocrLog = 'Ќе настроен (ключ: ${CloudOcrService.apiKey.isEmpty ? "пустой" : "есть"}, folder: ${CloudOcrService.folderId.isEmpty ? "пустой" : "есть"})';
      }
  ...
  String _extractItems(List<String> lines, String storeName) {
    final skipKeywords = ['итого', 'итог', 'сумма', 'к оплате', 'оплата', 'сдача',
      ...
      'чек', 'кассир', 'продавец', 'покупатель', 'спасибо', 'ждЄм', 'ждем',
      '---------', '=======', 'терминал', 'экземпл€р', 'аванс', 'предоплат'];
  ...
  String _findStoreName(List<String> lines) {
    ...
    final headerWords = ['кассовый', 'чека', 'чеком', 'сайт', 'тел', 'адрес',
      'огрн', 'эл', 'рисунок', '€ндекс', 'картинк'];

    final storeClues = ['ресторан', 'магазин', 'кафе', 'бар', 'столова€',
      'каль€нна€', 'кофейн', 'пиццер', 'аптек', 'спорт', 'fix', 'мега',
      'лента', 'ашан', 'п€терочк', 'магнит', 'вкусвилл', 'перекрЄсток',
    ...
    for (final line in lines) {
      ...
      if (skipExact.any((w) => lower == w || norm == w)) continue;
      if (headerWords.any((w) => lower.contains(w))) continue;
      if (RegExp(r'[а-€Є]{3,}').hasMatch(norm)) return _cleanStoreName(line);
    }
    for (final line in lines) {
      final lower = line.toLowerCase();
  ...
  String _cleanStoreName(String line) {
    ...
    s = s.replaceAll(RegExp(r'["Ђї""]'), '').trim();
  ...
  String? _detectCategory(String normalizedText, List<dynamic> categories) {
    final categoryClues = {
      'питание': ['ресторан', 'кафе', 'столова€', 'бар', 'кофейн', 'пиццер', 'еда', 'продукт', 'блюдо', 'порци', 'вино', 'пиво', 'кофе', 'чай', 'м€со', 'рыба', 'салат', 'суп', 'хлеб', 'молоко', 'сыр', 'колбас', 'напиток', 'сок', 'десерт', 'пицца', 'ролл', 'суши', 'бургер', 'картоф', 'свинин', 'курица', 'котлет', 'пельмен', 'блины', 'морожен', 'шоколад', 'торт', 'пирожн', 'бутерброд', 'азу', 'паста', 'макарон', 'масло', 'творог', '€иц', 'гов€дин', 'лосось', 'креветк', 'кальмар'],
      ...
      'домашнее хоз€йство': ['магазин', 'хоз', 'стройматер', 'мебель', 'обои', 'краск', 'ламинат'],
  ...
  List<double> _extractSignificantNumbers(String s) {
    var prepared = s.replaceAll('ќ', '0').replaceAll('о', '0');
  ...
  List<double> _extractNumbersAfterKeyword(String line) {
    ...
    var cleaned = numPart.replaceAll(' ', '').replaceAll(',', '.').replaceAll('ќ', '0').replaceAll('о', '0');
    if (cleaned.contains('.')) {
