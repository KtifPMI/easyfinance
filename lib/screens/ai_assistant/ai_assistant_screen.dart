...
class _AiAssistantScreenState extends State<AiAssistantScreen> {
  ...
  _ChatMessage _analyze(String query, FinanceStore store) {
    ...
    // ---- BALANCE ----
    if (_match(q, ['баланс', 'сколько денег', 'сколько всего', 'мой баланс', 'мои деньги', 'общий счЄт', 'balance', 'how much money', 'total'])) {
      return _ChatMessage(role: 'assistant', text: '${context.tr('ai.balance_title')}\n\n${accs.map((a) => 'Х ${a.name}: ${store.fmt(a.balance, fromCurrency: a.currency)}').join('\n')}\n\n${context.tr('ai.total')}: ${store.fmt(accountsTotal)}', navLabel: context.tr('ai.nav_accounts'), navRoute: '/accounts');
    }

    // ---- INCOME ----
    if (_match(q, ['доход', 'доходы', 'заработал', 'заработок', 'приход', 'сколько получил', 'income', 'earned', 'earnings', 'revenue', 'поступлени€'])) {
      ...
      final top = incomes.take(5).map((o) {
        ...
        return 'Х ${formatDate(o.date)}: ${cat?.name ?? 'Ч'} Ч ${store.fmt(o.amount, fromCurrency: store.accounts.where((a) => a.id == o.accountId).firstOrNull?.currency ?? 'RUB', date: o.date)}';
      }).join('\n');
    ...
    if (_match(q, ['расход', 'расходы', 'трат', 'потратил', 'ушло', 'списание', 'expense', 'spent', 'spending', 'cost', 'costs'])) {
      final top5 = topCat.take(5).map((e) => 'Х ${e.key}: ${store.fmt(e.value)} (${(e.value / mexp * 100).round()}%)').join('\n');
    ...
    // ---- CATEGORIES ----
    if (_match(q, ['категор', 'структура', 'на что', 'куда уход€т', 'разбивка', 'category', 'categories', 'breakdown', 'break down', 'where'])) {
      final all = topCat.map((e) => 'Х ${e.key}: ${store.fmt(e.value)} (${(e.value / mexp * 100).round()}%)').join('\n');
    ...
    // ---- SAVINGS ----
    if (_match(q, ['сбережен', 'накоплен', 'отложил', 'остаток', 'экономи€', 'свободн', 'saving', 'savings', 'leftover', 'remain', 'surplus'])) {
    ...
    if (_match(q, ['бюджет', 'лимит', 'план', 'превышен', 'перерасход', 'budget', 'limit', 'over', 'planned'])) {
      ...
      final parts = budgets.map((b) {
        ...
        return 'Х ${b.name ?? cat?.name ?? 'Ч'}: ${store.fmt(b.spent)} / ${store.fmt(b.limit)} ($pct%)$warn';
      }).join('\n');
    ...
    if (_match(q, ['цел', 'мечт', 'коплю', 'копить', 'накопить', 'goal', 'target', 'dream', 'saving for'])) {
      ...
      final parts = active.map((g) {
        ...
        final rec = m != null && m > 0 && pct < 80 ? ' Ч ${context.tr('goals.recommendation', namedArgs: {'amount': store.fmt(m)})}' : '';
        return 'Х ${g.title}: ${store.fmt(g.currentAmount)} / ${store.fmt(g.targetAmount)} ($pct%)$rec';
      }).join('\n');
    ...
    if (_match(q, ['совет', 'рекомендац', 'улучш', 'оптимиз', 'помощ', 'предлож', 'tip', 'recommend', 'advice', 'improve', 'suggest', 'help me'])) {
      ...
      final items = recs.map((r) => 'Х ${r.localizeTitle(context.tr(r.titleKey))}\n  ${r.localizeDesc(context.tr(r.descKey))}').join('\n\n');
    ...
    // ---- ACCOUNTS ----
    if (_match(q, ['счЄт', 'счет', 'счета', 'карт', 'кошел', 'account', 'accounts', 'card', 'wallet'])) {
      final aText = accs.map((a) => 'Х ${a.name} (${a.type == 'credit' ? context.tr('accounts.type.credit') : context.tr('accounts.type.cash')}): ${store.fmt(a.balance, fromCurrency: a.currency)}${a.isFavorite ? ' ?' : ''}').join('\n');
    ...
    // ---- TOTAL / SUMMARY ----
    if (_match(q, ['итог', 'обзор', 'сводка', 'всЄ', 'все', 'резюме', 'summary', 'overview', 'total', 'everything', 'all'])) {
    ...
    // ---- REPORT ----
    if (_match(q, ['отчЄт', 'отчет', 'аналитик', 'статистик', 'график', 'диаграм', 'report', 'analysis', 'stats', 'chart', 'graph'])) {
    ...
    // ---- SETTINGS ----
    if (_match(q, ['настройк', '€зык', 'тема', 'тЄмн', 'темн', 'пин', 'парол', 'валют', 'профил', 'settings', 'language', 'theme', 'dark', 'pin', 'password', 'profile', 'account info'])) {
