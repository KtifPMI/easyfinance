    ...
    if (tachRaw != null) {
      try {
        ...
        _serverFinHealth = FinHealthIndicators(
          ...
          finStateTip: t['finStateTip'] as String? ?? '',
          moneyTip: t['moneyTip'] as String? ?? '',
          budgetTip: t['budgetTip'] as String? ?? '',
          debtTip: t['debtTip'] as String? ?? '',
          incomeTip: t['incomeTip'] as String? ?? '',
...
Future<void> _saveCache() async {
    ...
    if (_serverFinHealth != null) {
      ...
      await prefs.setString(_ck('easyfinance_cached_tachometers'), jsonEncode({
        ...
        'debt': h.debt,
        'income': h.income,
        'finStateTip': h.finStateTip,
        'moneyTip': h.moneyTip,
        'budgetTip': h.budgetTip,
        'debtTip': h.debtTip,
        'incomeTip': h.incomeTip,
...
Future<void> setDisplayCurrency(String code) async {
  ...
  /// Сумма доходов за месяц по категориям — для отображения выполнения
  /// планов по доходам (аналог [monthSpentByCategory]).
  Map<String, double> monthEarnedByCategory([DateTime? month]) {
    final m = month ?? DateTime.now();
    final start = DateTime(m.year, m.month, 1);
    final end = DateTime(m.year, m.month + 1, 0, 23, 59, 59);
    final result = <String, double>{};
    for (final o in _operations) {
      if (o.isDeleted || o.type != 'income' || o.categoryId == null) continue;
      if (_inPeriod(o.date, start, end)) {
        result[o.categoryId!] = (result[o.categoryId!] ?? 0) + _amountInRub(o);
      }
    }
