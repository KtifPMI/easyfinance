...
В API **нет методов `*.delete`** (accounts/categories/tags/operationPatterns).
Удаление выполняется через соответствующий `*.set` с проставленным полем
`deleted_at`, равным времени удаления:
```
deleted_at = 2012-12-12T00:57:58+0200
```
Сервер делает soft-delete. Для счетов также используется `state = "2"`.
...
**Методов `*.delete` в API нет** — удаление везде через `*.set` с `deleted_at`
(или `state="2"` для счетов).
